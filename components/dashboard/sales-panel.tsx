...
export function SalesPanel({
  ...
  const currentLabel =
    PERIODS.find((p) => p.key === range)?.label ?? "Últimos 30 dias";

  const data = useMemo(() => {
    const { start, end } = getRange(range);
    return buildSeries(vendas, start, end);
  }, [vendas, range]);

  const temDados = data.some((d) => d.pedidos > 0);
  const tituloPeriodo = currentLabel
    .replace("Últimos ", "últimos ")
    .replace("Hoje", "hoje")
    .replace("Este mês", "este mês")
    .replace("Mês anterior", "mês anterior");
