-- 0074 — Corrige a criação de promissória para respeitar o constraint de valor_total > 0.
...
begin
  ...
  if p_promissoria_id is null then
    insert into public.promissorias(
      ...
      p_cliente_id, greatest(coalesce(p_valor_base,0),0.01), p_parcelas, 'em_aberto', nullif(trim(coalesce(p_observacao,'')),''),
