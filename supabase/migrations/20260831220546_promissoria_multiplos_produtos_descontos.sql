...
-- preco_original fica nulo nos itens legados. Assim a migração não reescreve
-- nenhum dado operacional; a RPC e a interface usam preco_unitario como fallback.
do $$
begin
  if not exists (
    select 1 from pg_constraint
     where conname = 'promissoria_itens_desconto_tipo_check'
       and conrelid = 'public.promissoria_itens'::regclass
  ) then
    alter table public.promissoria_itens
      add constraint promissoria_itens_desconto_tipo_check
...
begin
  if not exists (
    select 1 from pg_constraint
     where conname = 'promissoria_itens_precos_descontos_check'
       and conrelid = 'public.promissoria_itens'::regclass
  ) then
    alter table public.promissoria_itens
      add constraint promissoria_itens_precos_descontos_check
      check (
        preco_unitario >= 0
        and desconto_valor >= 0
        and desconto_percentual between 0 and 100
        and (
          (preco_original is null and desconto_tipo is null and desconto_valor = 0 and desconto_percentual = 0)
          or (
            preco_original >= 0
            and preco_unitario <= preco_original
            and desconto_valor <= preco_original
          )
        )
      );
  end if;
...
begin
  ...
  if p_promissoria_id is null then
    insert into public.promissorias(
      ...
      p_cliente_id, 0, p_parcelas, 'em_aberto',
...
begin
  ...
  if v_org is null then raise exception 'Empresa não identificada'; end if;
  if nullif(p_forma,'') is not null and p_forma not in ('pix','dinheiro','cartao') then
  ...
  if p_idempotency_key is not null and exists (
    select 1
     ...
     where idempotency_key=p_idempotency_key
       ...
       and promissoria_id=p_promissoria_id
  ) then
    select coalesce(valor_total,0) into v_total
      from public.promissorias
     where id=p_promissoria_id and organization_id=v_org;
    select coalesce(sum(valor),0) into v_pago
      from public.promissoria_pagamentos
     where promissoria_id=p_promissoria_id and organization_id=v_org;
  ...
  select valor_total,status into v_total,v_status
   ...
   where id=p_promissoria_id and organization_id=v_org
   for update;
  if v_total is null then raise exception 'Promissória não encontrada'; end if;
  if v_status='cancelado' then raise exception 'Promissória cancelada não aceita pagamento'; end if;
  if v_status='pago' then raise exception 'Promissória já está quitada'; end if;
  if coalesce(p_valor,0)<=0 then raise exception 'Informe um valor de pagamento válido'; end if;
  ...
  insert into public.promissoria_pagamentos(
    ...
    v_org,p_promissoria_id,p_valor,v_hoje,nullif(p_forma,''),p_obs,'parcela',p_idempotency_key
  )
  on conflict (idempotency_key) where idempotency_key is not null do nothing;

  get diagnostics v_inseriu=row_count;
  if v_inseriu=0 then return greatest(v_saldo,0); end if;
  ...
  v_saldo := v_total-v_pago;
  if v_saldo<=0 then
    update public.promissorias set status='pago'
     where id=p_promissoria_id and organization_id=v_org;
  end if;
  ...
  perform public.log_auditoria(
    ...
    jsonb_build_object(
      'valor',p_valor,'saldo',greatest(v_saldo,0),
      'forma',nullif(p_forma,''),'data',v_hoje
    )
  );
...
revoke all on function public.registrar_pagamento_promissoria(uuid,numeric,text,text,text) from public;
grant execute on function public.registrar_pagamento_promissoria(uuid,numeric,text,text,text) to authenticated;
