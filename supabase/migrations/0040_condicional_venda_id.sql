-- ─────────────────────────────────────────────────────────────────────────────
-- Correção final P1 — #9 Histórico do condicional
--
-- Para o condicional finalizado mostrar a VENDA GERADA (total, desconto, itens),
-- falta um vínculo direto: a `converter_condicional_venda` criava a venda mas só
-- registrava o `venda_id` na auditoria. Esta migration:
--   • adiciona `condicionais.venda_id` (FK para vendas);
--   • recria a RPC para gravar `venda_id` no condicional ao finalizar;
--   • faz backfill do vínculo para condicionais já convertidos, a partir do
--     log de auditoria ('condicional_convertido').
--
-- A reconstrução de vendido/devolvido por peça é feita no front
-- (lib/condicional-utils.ts) a partir do ledger de estoque + itens da venda.
--
-- SECURITY INVOKER, atômica, idempotente e reversível.
--
-- Como reverter:
--   -- restaurar converter_condicional_venda da 0037 (sem set venda_id);
--   alter table public.condicionais drop column if exists venda_id;
-- ─────────────────────────────────────────────────────────────────────────────

alter table public.condicionais
  add column if not exists venda_id uuid references public.vendas (id);

create index if not exists condicionais_venda_idx
  on public.condicionais (venda_id);

-- Recria a RPC (idêntica à 0037, mas gravando venda_id no condicional).
...
begin
  ...
  -- #9: grava o vínculo com a venda gerada (para o histórico da finalização).
...
-- Backfill: vincula condicionais já convertidos à venda registrada na auditoria.
update public.condicionais c
set venda_id = sub.venda_id
from (
  select distinct on (registro_id)
    registro_id,
    nullif(dados->>'venda_id', '')::uuid as venda_id
  from public.audit_logs
  where acao = 'condicional_convertido'
    and dados ? 'venda_id'
    and nullif(dados->>'venda_id', '') is not null
  order by registro_id, created_at desc
) sub
where c.id = sub.registro_id
  and c.venda_id is null;
